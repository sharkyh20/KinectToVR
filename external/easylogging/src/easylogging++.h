...
inline T_Ptr* get(const T_Key& uniqKey) {
    typename Registry<T_Ptr, T_Key>::iterator it = this->list().find(uniqKey);
...
for (typename Registry<T_Ptr, T_Key>::const_iterator it = sr.cbegin(); it != sr.cend(); ++it) {
...
public:
    ...
    friend inline base::type::ostream_t& operator<<(base::type::ostream_t& os, const RegistryWithPred& sr) {
        for (typename RegistryWithPred<T_Ptr, Pred>::const_iterator it = sr.list().begin(); it != sr.list().end(); ++it) {
...
if (ptr) {
    typename RegistryWithPred<T_Ptr, Pred>::iterator iter = this->begin();
...
inline T_Ptr* get(const T& arg1, const T2 arg2) {
    typename RegistryWithPred<T_Ptr, Pred>::iterator iter = std::find_if(this->list().begin(), this->list().end(), Pred(arg1, arg2));
