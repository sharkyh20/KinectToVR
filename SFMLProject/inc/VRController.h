...
public:
		...
		if (m_HMDSystem != nullptr)
		{
			...
			else
			{
				lastStateValid = m_HMDSystem->GetControllerStateWithPose(
					vr::ETrackingUniverseOrigin::TrackingUniverseStanding, controllerID, &state_, sizeof(state_),
					&controllerPose);
				if (lastStateValid)
				{
					prevState_ = state_;
				...
				if (m_HMDSystem->GetControllerRoleForTrackedDeviceIndex(controllerID) ==
					vr::TrackedControllerRole_LeftHand)
				{
					vr::TrackedDevicePose_t trackedDevicePose[vr::k_unMaxTrackedDeviceCount];
					m_HMDSystem->GetDeviceToAbsoluteTrackingPose(vr::TrackingUniverseStanding, 0, trackedDevicePose,
	...
	sf::Vector2f GetControllerAxisValue(vr::EVRButtonId buttonId)
	{
		if (lastStateValid)
		{
			...
			uint64_t axisId = static_cast<uint64_t>(buttonId) - static_cast<uint64_t>(vr::k_EButton_Axis0);
			switch (axisId)
			{
			case 0: axis_value = sf::Vector2f(state_.rAxis[0].x, state_.rAxis[0].y);
				break;
			case 1: axis_value = sf::Vector2f(state_.rAxis[1].x, state_.rAxis[1].y);
				break;
			case 2: axis_value = sf::Vector2f(state_.rAxis[2].x, state_.rAxis[2].y);
				break;
			case 3: axis_value = sf::Vector2f(state_.rAxis[3].x, state_.rAxis[3].y);
				break;
			case 4: axis_value = sf::Vector2f(state_.rAxis[4].x, state_.rAxis[4].y);
				break;
			}
			return axis_value;
		}
	...
	void UpdateTrigger() //TODO test returned values, as they may be acidentally triggering on touch
	{
		if (lastStateValid)
		{
			...
			gripOn = (state_.ulButtonPressed & ButtonMaskFromId(vr::k_EButton_Grip)) != 0; //for grip
		}
	...
	bool GetTrigger()
	{
		if (lastStateValid) return triggerOn;
		return false;
	}

	bool GetTriggerDown()
	{
		if (lastStateValid) return triggerOn && !triggerPrevOn;
		return false;
	}
	...
	bool GetGrip()
	{
		if (lastStateValid) return gripOn;
		return false;
	}

	bool GetGripDown()
	{
		if (lastStateValid) return gripOn && !gripPrevOn;
		return false;
	}
	...
	{
		if (lastStateValid)
			return (state_.ulButtonPressed & ButtonMaskFromId(buttonId)) != 0;
		return false;
	}
	...
	bool GetPressDown(vr::EVRButtonId buttonId)
	{
		if (lastStateValid)
			return (state_.ulButtonPressed & ButtonMaskFromId(buttonId)) != 0
				&& (prevState_.ulButtonPressed & ButtonMaskFromId(buttonId)) == 0;
		return false;
	}
	...
	{
		if (lastStateValid)
			return (state_.ulButtonTouched & ButtonMaskFromId(buttonId)) != 0;
		return false;
	}
	...
	bool GetTouchDown(vr::EVRButtonId buttonId)
	{
		if (lastStateValid)
			return (state_.ulButtonTouched & ButtonMaskFromId(buttonId)) != 0
				&& (prevState_.ulButtonTouched & ButtonMaskFromId(buttonId)) == 0;
		return false;
	}
		...
		if (m_HMDSystem != nullptr && lastStateValid)
		{
			if (controllerPulse.active)
			{
				if (controllerPulse.elapsed < controllerPulse.length)
				{
					...
					m_HMDSystem->TriggerHapticPulse(controllerID, 0, lerp(0, 3999, controllerPulse.strength));
					// replace 0 with axisID, removed for testing
				}
				else
...
private:
	...
	bool OverrideTrackedPosWithKinect = false;
	// Mainly for MR controllers, when tracking lost, if true, then the kinect hand position will override MR hand position
};
