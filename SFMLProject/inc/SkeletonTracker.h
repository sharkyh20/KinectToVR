...
private:
	...
	void applyKinectArrowCalibrationToTracker(KVR::KinectTrackedDevice& device, vr::HmdQuaternion_t& rotation,
	                                          vr::HmdVector3d_t& position)
	{
		...
		vr::HmdVector3d_t rotatedPos = vrmath::quaternionRotateVector(KinectSettings::kinectRepRotation, position,
		                                                              false);
	...
	void updateDevicePosePosition(KVR::KinectTrackedDevice& device, vr::DriverPose_t& pose,
	                              vr::HmdVector3d_t rotatedPos)
	{
	...
	vr::DriverPose_t generateKinectPose(KVR::KinectTrackedDevice& device, vr::HmdQuaternion_t& rotation,
	                                    vr::HmdVector3d_t& position)
	{
	...
	void updatePoolWithKinectJoint(KinectHandlerBase& kinect, KVR::KinectTrackedDevice& device)
	{
		...
		if (!usingSkeletonPosition
			...
			return;
		KVR::TrackedDeviceInputData data = defaultDeviceData(static_cast<int>(device.joint0.joint));

		data.deviceId = usingSkeletonPosition ? device.positionDevice_gId : device.rotationDevice_gId;
		// Doesn't need to be checked, as if it's made it past the initial check, it's going to be one or the other ID
