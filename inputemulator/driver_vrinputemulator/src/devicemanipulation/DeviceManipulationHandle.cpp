	...
	if (m_deviceMode == 1) { // fake disconnect mode
		...
		if (m_offsetsEnabled) {
			...
			if (m_worldFromDriverTranslationOffset.v[0] != 0.0 || m_worldFromDriverTranslationOffset.v[1] != 0.0 || m_worldFromDriverTranslationOffset.v[2] != 0.0) {
				...
				//m_worldFromDriverTranslationOffset.v[0] = 0;
				//m_worldFromDriverTranslationOffset.v[1] = 0;
				//m_worldFromDriverTranslationOffset.v[2] = 0;
			...
			if (m_driverFromHeadTranslationOffset.v[0] != 0.0 || m_driverFromHeadTranslationOffset.v[1] != 0.0 || m_driverFromHeadTranslationOffset.v[2] != 0.0) {
				...
				//m_driverFromHeadTranslationOffset.v[0] = 0;				
				//m_driverFromHeadTranslationOffset.v[1] = 0;
				//m_driverFromHeadTranslationOffset.v[2] = 0;
			...
			if (m_deviceTranslationOffset.v[0] != 0.0 || m_deviceTranslationOffset.v[1] != 0.0 || m_deviceTranslationOffset.v[2] != 0.0) {
				...
				//m_deviceTranslationOffset.v[0] = 10;
				//m_deviceTranslationOffset.v[1] = 10;
				//m_deviceTranslationOffset.v[2] = 10;
...
void DeviceManipulationHandle::ll_sendPoseUpdate(const vr::DriverPose_t& newPose) {
	if (m_deviceDriverInterfaceVersion == 4) {
		//IVRServerDriverHost004Hooks::trackedDevicePoseUpdatedOrig(m_deviceDriverHostPtr, 0, newPose, sizeof(vr::DriverPose_t));
	} else if (m_deviceDriverInterfaceVersion == 5) {
		//IVRServerDriverHost005Hooks::trackedDevicePoseUpdatedOrig(m_deviceDriverHostPtr, 0, newPose, sizeof(vr::DriverPose_t));
	}
}
...
float OVX, OVY, OVZ, OVRX, OVRY, OVRZ, OVR;

void pipeof() {
	while (1) {
		HANDLE pipeOf = CreateNamedPipe(TEXT("\\\\.\\pipe\\LogPipeOf"), PIPE_ACCESS_INBOUND | PIPE_ACCESS_OUTBOUND, PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE, 1, 1024, 1024, 120 * 1000, NULL);
		if (pipeOf == INVALID_HANDLE_VALUE)
		{
			LOG(ERROR) << ("PipeError: %d", GetLastError());
		}
		...
		ServerDriver::started = true;
		CloseHandle(pipeOf);

		std::string OfS = OfD;

		OVX = (float)nstr(OfS, "X") / (float)10000;
		OVY = (float)nstr(OfS, "Y") / (float)10000;
		OVZ = (float)nstr(OfS, "Z") / (float)10000;
		OVR = (float)nstr(OfS, "R") / (float)10000;
...
void pipenrot() {
	while (1) {
		...
		if (pipeIchi != INVALID_HANDLE_VALUE) 
		{
			ConnectNamedPipe(pipeIchi, NULL);
			ReadFile(pipeIchi, IchiD, 1024, &Ichi, NULL);
		...
		float DVX = (float)nstr(IchiS, "X") / (float)10000;
		float DVY = (float)nstr(IchiS, "Y") / (float)10000;
		float DVZ = (float)nstr(IchiS, "Z") / (float)10000;
		...
		ServerDriver::nrotT.x = OVRX;
		ServerDriver::nrotT.y = OVRY;
		ServerDriver::nrotT.z = OVRZ;
...
void DeviceManipulationHandle::RunFrame() {
	...
	enableOffsets(true);
	deviceTranslationOffset() = ServerDriver::nposT;
